...
function func(array, element){
    ...
    if(new_array.includes(element)){
        new_array.indexOf(element) ? new_array.splice(new_array.indexOf(element) - 1, 3) : new_array.splice(new_array.indexOf(element), 2)
        return new_array
    }
}
...
let new_fruits = fruit.slice()
for (let i = fruit.length; i > 0; i--){
    if(i % 2){
        new_fruits.splice(i, 1)
    }
}
...
let findedStudent = students.filter(item => item.name === 'Sam')
